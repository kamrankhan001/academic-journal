...
@section('title', 'Edit Journal - Academic Journal')
...
@section('content')
    <div class="bg-white rounded-xl shadow-soft border border-gray-200 p-6">
        <form action="{{ route('author.journals.update', $journal) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="abstract" 
                              ...
                              placeholder="Provide a brief summary of your research">{{ old('abstract', $journal->abstract) }}</textarea>
                    @error('abstract')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select id="tags-select" name="tags[]" multiple class="hidden">
                        @foreach($tags as $tag)
                            @php
                                $selected = in_array($tag->id, old('tags', $selectedTags ?? []));
                            @endphp
                            <option value="{{ $tag->id }}" 
                                    data-name="{{ $tag->name }}" 
                                    {{ $selected ? 'selected' : '' }}>
