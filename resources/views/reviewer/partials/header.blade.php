<header class="bg-white border-b border-gray-200 h-16 flex items-center px-6">
    <div class="flex items-center justify-between w-full">
        ...
        <div class="flex items-center">
            ...
            <h2 class="text-lg font-semibold text-gray-800 hidden md:block">@yield('page-title', 'Reviewer Dashboard')</h2>
        </div>
            ...
            <div class="relative" id="notificationsDropdown">
                <button onclick="toggleDropdown('notifications')" class="text-gray-500 hover:text-[#86662c] relative">
                    ...
                    @php
                        ...
                        $pendingReviews = Auth::user()->reviewerProfile ? 
                            Auth::user()->reviewerProfile->reviewAssignments()
                        ...
                        @forelse(Auth::user()->notifications()->latest()->take(5)->get() as $notification)
                            ...
                            <a href="{{ $data['action_url'] ?? '#' }}"
                                ...
                                <p class="text-sm font-medium text-gray-800">
                                    {{ $data['type'] === 'review_invitation' ? 'New Review Invitation' : ($data['type'] === 'assignment_accepted' ? 'Review Accepted' : ($data['type'] === 'reminder' ? 'Reminder' : 'Notification')) }}
                                </p>
            ...
            <div class="relative" id="userDropdown">
                <button onclick="toggleDropdown('user')" class="flex items-center space-x-3 focus:outline-none">
                    <div class="text-right hidden md:block">
                        ...
                        <p class="text-xs text-gray-500">{{ Auth::user()->reviewerProfile->academic_degree ?? 'Reviewer' }}</p>
                    </div>
...
<script>
    ...
    function markAllAsRead() {
        fetch('/reviewer/notifications/mark-all-read', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
    ...
    function updateNotificationCount() {
        fetch('/reviewer/notifications/unread-count', {
            ...
            .then(data => {
                ...
                const sidebarBadge = document.querySelector('#sidebar .fa-bell + span, #sidebar .fa-bell ~ span');
                if (sidebarBadge) {
                    if (data.count > 0) {
